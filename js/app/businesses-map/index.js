...
const markers = window.__businesses.map(business => {

  const pos = { lat: business.address.geo.latitude, lng: business.address.geo.longitude }
  ...
  if (window.parent) {
    el.on('click', '.business-map-link', function(e) {
      e.preventDefault()
      window.parent.document.location.replace($(this).attr('href'))
    })
  }

  el.html(`<a href="${ business.url }" class="business-map-link">${ business.name }</a>`)
...
if (markers.length > 0) {
  const businessesLayer = L.layerGroup(markers)
    businessesLayer.addTo(map)
