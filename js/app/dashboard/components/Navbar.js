                ...
                <div className="dashboard__date">
                  ...
                  <div className="dashboard__date-picker">
                    <DatePicker
                      ...
                      onChange={(date) => {
                        if (date) {
                          window.location.href = window.Routing.generate('dashboard_fullscreen', {
...
function mapStateToProps(state) {
  ...
  return {
    date: selectedDate,
    prev: window.Routing.generate('dashboard_fullscreen', {
      date: moment(selectedDate).subtract(1, 'days').format('YYYY-MM-DD'),
      nav: state.config.nav
    }),
    next: window.Routing.generate('dashboard_fullscreen', {
