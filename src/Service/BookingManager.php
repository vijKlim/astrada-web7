...
class BookingManager
{
    ...
    public function __construct(
        ...
        $this->timesMax = $parameters["astrada_time_max"];
        $this->hoursAvailable = $parameters["astrada_time_hours_available"];
        ...
        $this->expirationDelay = $parameters["astrada_booking_expiration_delay"];
        ...
        $this->minPrice = $parameters["astrada_booking_price_min"];

        $this->maxPerPage = $parameters["astrada_dashboard_max_per_page"];
        $this->defaultListingStatus = $parameters["astrada_listing_availability_status"];
        $this->invoiceBegin = $parameters["astrada_booking_invoice_begin"];
    }
    ...
    public function initBookingDatesInDayMode(Booking $booking)
    {
        ...
        $minStartDay->add(new DateInterval('P1D'));//+1 день
        $minStartDay->setTime(0, 0, 0);

        $maxEndDay = new DateTime();
        $maxEndDay->add(new DateInterval('P1M'));//+ 1 есяц
...
//            echo $timeRange->getStartMinute() . ' / ' . $timeRange->getEndMinute() . '<br>';
    ...
    public function create(Booking $booking)
    {
        if (in_array($booking->getStatus(), Booking::$newableStatus)) {
            ...
            $this->emailManager->sendBookingRequestMessageToOfferer($booking);
            $this->emailManager->sendBookingRequestMessageToAsker($booking);
...
//                throw new \Exception($e);
    ...
    {
        return $this->em->getRepository(ListingAvailability::class);
    }
